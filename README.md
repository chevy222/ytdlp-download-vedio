...
| 多窗口    | 可以同时开多个窗口下载：每个窗口有自己的编号文件夹（靠 `mkdir` 原子抢号，不靠随机数），互不撞名、互不删对方文件（见[多窗口同时下载](#多窗口同时下载)）                                 |
...
| 退出     | 循环模式下输入 `q` / `quit` / `exit`；输入 `c` 清掉 `%TEMP%` 下的临时文件夹（不会退出，见[多窗口](#多窗口同时下载)）                                                                  |
...
============================================================
  ...
  f URL = list formats only    q = quit    c = clear temp files
============================================================
...
能在这个工具流程里出现的临时文件一共三类：① 脚本自己在 `%TEMP%` 下开的**一个私有文件夹**（装 4 个记录文件）、② REBUILD 的中间产物、③ yt-dlp 自己在**输出目录**里的分片/半成品。前两类脚本会自己收尾，第三类由 yt-dlp 收尾（中断时不会）。

**① `%TEMP%\ytdlp_<N>\` —— 每个窗口一个私有文件夹**

`<N>` 不是随机数，而是**原子抢来的编号**（1、2、3……第一个没人占的号）。文件夹里固定 4 个文件：

| 文件 | 谁写的 | 什么时候写 | 什么时候被删 |
| -------------------------------------- | -------------------- | ------------------------- | --------------------------------- |
| `last.txt` | yt-dlp | 下载**成功**后（`--print-to-file after_move`） | **本轮结束时随文件夹一起删**（`:NEXT_ROUND`）；`:GOT_URL` / `:END` 各有兜底 |
| `h.txt` | ffprobe（PROBE_HEIGHT） | 进入探测且 `TRANSCODE_ON=1` 时 | **PROBE_HEIGHT 一返回就删** |
| `cnt.txt` | ffprobe（PROBE_AUDIO） | 进入探测且 `AMPLIFY_ON=1` 时 | **PROBE_AUDIO 一返回就删** |
| `vol.txt` | ffmpeg（`volumedetect`） | **必须**先通过“音轨数 = 1”这一关才会写 | **PROBE_AUDIO 一返回就删** |

所以：

- **看不到 `vol.txt` 是正常的**——它排在“音轨数检查”之后。只要 ffprobe 读不出音轨（文件打不开 / 输出为空），`ACNT` 就是 0，脚本在写 `vol` 之前就 `exit /b 0` 了，于是只留下另外 3 个。这正是 emoji 标题那次的现场：`ytdlp_h_6442.txt` 和 `ytdlp_cnt_6442.txt` 都是 **0 字节**，`ytdlp_last_6442.txt` 里是 UTF-8 的完整路径，而 `ytdlp_vol_6442.txt` 压根没生成。
- **停在提示符上的窗口，临时文件数恒为 0**。每轮一结束（成功、yt-dlp 报错、"已下载跳过"、甚至只列格式）都会经过 `:NEXT_ROUND`，那里把 4 个文件和文件夹一起清掉；探测用的三个更早，一读完就删。实测：一轮下载 + 转码跑完、窗口停在 `Enter URL:` 时，`%TEMP%` 里已经没有这个窗口的文件夹了。
- **异常退出**（关窗口、Ctrl-C、崩溃在下载/探测中途）时 bat 没机会跑清理，文件夹会留下（里面最多 4 个几百字节的小文件）。脚本**不会自动去删它**（原因见下面「多窗口」），启动时只在横幅下提示一句：

  ```
    Note: ytdlp temp folders from an earlier run are still in the temp
          folder. Type c and press Enter to clear them.
  ```

  这时在提示符输入 **`c`** 回车即可清空（`c` 也是唯一的跨窗口删除入口，见下）。

#### 多窗口同时下载

- **每个窗口有自己的编号文件夹，绝不可能撞名。** 编号靠 `mkdir` 原子抢占：`mkdir` 遇到已存在的目录会失败，所以窗口 A 抢到 1、窗口 B 自动退到 2，一个编号只会属于一个窗口。
- **为什么用抢编号而不是随机数**（实测纠正）：这台机器上 `%RANDOM%` **完全不是随机的**——同时启动 4 个 cmd，打出来的序列一模一样，隔 50ms 再启动还是同一串：

  ```
  7617 23633 1235 23274     <- 4 个同时启动的实例，输出完全相同
  ```

  它的种子精度是「秒」，不是「每进程」。旧版本用 `ytdlp_last_<RANDOM>.txt` 这类命名，**两个窗口一直在共用同一组文件**，实测到两种破坏：先跑完探测的窗口会**删掉另一个窗口的探测文件**（那个窗口就静默地不做音量归一化/降分辨率），更严重的是两边读**同一个** `last.txt`，**一个窗口可能去后处理另一个窗口的视频**（实测出现过 ffmpeg 产出 0 字节、转码失败）。
- **脚本没有任何"启动时顺手清一遍"的跨窗口删除**——刻意如此。cmd 无法可靠区分"另一个窗口正在用这个"和"上次崩溃留下的垃圾"，自动清就必然偶尔动到活窗口的文件。所以脚本只碰自己的文件夹。
- 唯一会跨窗口删除的是提示符下的 **`c`** 命令（删除 `%TEMP%` 下**所有** `ytdlp_*` 文件夹）。它只在手动输入时执行，所以**别在另一个窗口正在下载时按 `c`**。平时根本不用按：正常情况下每一轮都会自己清干净。
- 实测：两个窗口同时下载同一个 2560x1440 源，各自抢到 `ytdlp_1` / `ytdlp_2`，两边都正确走完「降分辨率 + 音量归一化」（产物都是 `hevc 1920x1080 + aac`），结束后 `%TEMP%` 里两个文件夹都已清空。
...
- 中断则会留下，而且脚本**不清扫**这些（只清自己那个 `%TEMP%\ytdlp_<N>\` 文件夹），看到直接删即可
...
Enter URL: <粘贴链接>
Enter URL: <再来一个>
Enter URL: c     <-- 清掉历史残留的临时文件（不退出，可继续下）
Enter URL: q     <-- 退出
```
...
| 用 `%RANDOM%` 给临时文件/目录去重，以为"两个实例不会撞" | 同一秒内启动的 cmd **打出完全相同的序列**（实测 4 个实例都是 `7617 23633 1235 23274`）→ 两个窗口共用文件名 → 一个窗口删掉/读走另一个窗口的探测文件，甚至后处理错文件 | 要唯一就别用随机数：**`mkdir` 是原子的**（目录已存在就失败），从 1 开始抢第一个没人占的编号即可 |
...
- **窗口被中途杀死会留下一个 `%TEMP%\ytdlp_<N>\` 文件夹**：脚本刻意不做自动跨窗口清扫（无法区分"别人正在用"和"上次崩溃"，误删会静默毁掉别人的一次转码），所以残留要自己处理——启动时会提示，提示符输入 `c` 即可清掉所有 `ytdlp_*` 文件夹。注意 `c` 是跨窗口的，**别在另一个窗口正在下载时按**
- **同一秒内启动多个窗口不再互相影响，但 `c` 仍然会**：编号文件夹解决了撞名问题，`c` 属于手动操作，语义上就是"清掉所有窗口的临时目录"
